...
def write_json(payload):
    with open("eth.json", "w") as f:
        json.dump(payload, f, indent=2)
    print("💾 eth.json written successfully.")

if __name__ == "__main__":
    print("🚀 Running ETH Band Monitor...")
    ...
    last = result["last"]
    msg = [
        f"{result['emoji']} ETH/USDT 5m — {result['prediction']}",
    ...
    if result["cross"] in ("ABOVE", "BELOW"):
        ...
        msg.append("Open ChatGPT for full analysis → "
                   "chat.openai.com/?model=gpt-5&prompt=Fetch+latest+ETH+5-min+Bollinger+analysis")
